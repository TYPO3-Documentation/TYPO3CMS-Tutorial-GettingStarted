...
container. Add a basic :ref:`site configuration <site-configuration>`
for this page.

Create a new TypoScript template record on this page.
...
6.  We use the :confval:`stdWrap <t3tsref:text-stdwrap>` property of the TEXT
    ...
    In this function we use the option wrap :ref:`wrap <t3tsref:stdwrap-wrap>`.
...
5.  Uses the stdWrap property :ref:`field <t3tsref:stdwrap-field>` to fetch
    the field `title` from the database record
    of the current page.
6.  Uses the stdWrap property :ref:`wrap <t3tsref:stdwrap-wrap>` to wrap the
